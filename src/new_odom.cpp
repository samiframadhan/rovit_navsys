...
double radius = 0.16;                              //Wheel radius, in m
...
void handle_speed( const rovit_navsys::debug& speed) {
  speed_act_left = trunc(speed.leftSpeed*100)/100;
  ROS_INFO("speed left : %f", speed_act_left);
  speed_act_right = trunc(speed.rightSpeed*100)/100;
  ROS_INFO("speed right : %f", speed_act_right);
  speed_dt = speed.data[4];
  speed_time = speed.header.stamp;
}

int main(int argc, char** argv){
  ros::init(argc, argv, "odom_from_arduino");

  ros::NodeHandle n;
  ros::NodeHandle nh_private_("~");
  ros::Subscriber sub = n.subscribe("debug/data", 50, handle_speed);
  ...
  //char odom[] = "/odom_enc";
  char odom[] = "odom";
  //Uncomment it if you not want to fuse the encoder

  ros::Duration d(1.0);
  nh_private_.getParam("publish_rate", rate);
  nh_private_.getParam("publish_tf", publish_tf);
  nh_private_.getParam("turn_covariance", zyaw_covariance);
